...
let projects = [
  {
    ...
    url: "https://www.automa-web.dev.br/#portfolio",
  },
];
...
const fanLight2 = new THREE.PointLight(0x00ff00, 30, 0.14);
fanLight2.position.set(-0.15, 0.29, 0.92); // Ajuste a posição aqui
scene.add(fanLight2);

const fanLight3 = new THREE.PointLight(0x00ff00, 30, 0.2);
fanLight3.position.set(0.15, 0.3, 0.94); // Ajuste a posição aqui
scene.add(fanLight3);

const fanLight4 = new THREE.PointLight(0x00ff00, 30, 0.2);
fanLight4.position.set(0.15, 0.15, 0.94); // Ajuste a posição aqui
scene.add(fanLight4);

const fanLight5 = new THREE.PointLight(0x00ff00, 30, 0.2);
fanLight5.position.set(0.15, 0.0, 0.94); // Ajuste a posição aqui
scene.add(fanLight5);

const fanLight6 = new THREE.PointLight(0xffffff, 10, 0.4);
...
function loadIntroText() {
  ...
  loader.load("fonts/unione.json", function (font) {
    ...
    const titleGeo = new TextGeometry("Francilino Neto", {
      font: font,
      size: 0.08,
      height: 0.01,
    });
    ...
    titleText.position.set(-0.255, 0.55, 0.8);
    scene.add(titleText);
  });
  ...
  loader.load("fonts/helvatica.json", function (font) {
    ...
    const subTitleGeo = new TextGeometry(
      "Python / Web Scraping / Automacao Web / Reac.Js / React Native / Java Script / HTML & CSS / Flask API / Analise de Dados...",
      {
        font: font,
        size: 0.018,
        height: 0,
    ...
    subtitleText.position.set(-0.255, 0.5, 0.91);
...
function enableCloseBtn() {
  ...
  console.log("aqui arrombado");
}

function enableCloseBtn1() {
  document.getElementById("close-btn").style.display = "block";
  console.log("aqui arrombado");
}
...
let defaultCameraPos2 = {
  x: 0.15,
  y: 0.113,
  z: 0,
};
let defaultCamerRot2 = {
  x: -1.55,
  y: 0,
  z: 0.1,
};
...
let defaultCameraPos1 = {
  x: 0.2,
  y: 0.3,
  z: 0.3,
};
let defaultCamerRot1 = {
  x: 0,
  y: 1.5,
  z: 0,
};

function cameraToAbout2() {
  if (!bookCover) return;
...
function init3DWorldClickListeners() {
  ...
  window.addEventListener("click", function (e) {
    ...
    intersects.forEach((intersect) => {
      if (
        intersect.object.name === "Monitor" ||
        intersect.object.name === "Pc"
      ) {
        disableOrbitControls();
        cameraToAbout2();
        gsap.delayedCall(1.5, enableCloseBtn1);
      }
      if (
        intersect.object.name === "Mobile" ||
        intersect.object.name === "Mobile Screen"
      ) {
        disableOrbitControls();
        cameraToAbout1();
        gsap.delayedCall(1.5, enableCloseBtn1);
